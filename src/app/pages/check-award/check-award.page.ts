...
export class CheckAwardPage implements OnInit {
  ...
  async getData() {
    ...
    const data = await firstValueFrom(
      this.http
        .get<ApiResponse>(`https://lotto.api.rayriffy.com/latest`)
  ...
  checkWonPrizes() {
    ...
    const uniqueNumbers = [...new Set(numberStrs)].filter(
    ...
    this.thePrizes?.forEach((prize) => {
      ...
      if (
        [
          ...
          'prizeForth',
          'prizeFifth',
        ].some((fullCheck) => prize.id === fullCheck)
      ) {
      ...
      if (prize.id === 'runningNumberFrontThree') {
        wonNumbers = prize.number.filter((num) =>
          uniqueNumbers
            .map((uniqueNumber) => uniqueNumber.slice(0, 3))
            .includes(num)
        );
      }
      ...
      if (prize.id === 'runningNumberBackThree') {
        wonNumbers = prize.number.filter((num) =>
          uniqueNumbers
            .map((uniqueNumber) => uniqueNumber.slice(3, 6))
            .includes(num)
        );
      }
      ...
      if (prize.id === 'runningNumberBackTwo') {
        wonNumbers = prize.number.filter((num) =>
          uniqueNumbers
            .map((uniqueNumber) => uniqueNumber.slice(4, 6))
            .includes(num)
        );
      }
...
//         reward: '200000',
//         amount: 5,
//         number: ['062041', '268817', '644812', '647084', '782648'],
//       },
//       {
//         id: 'prizeThird',
//         name: 'รางวัลที่ 3',
